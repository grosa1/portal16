...
angular
    .module('portal')
    .controller('dataValidatorLegacyKeyCtrl', dataValidatorKeyCtrl);
...
function dataValidatorKeyCtrl($http, $stateParams, $state, $timeout, DwcExtension, Remarks, $location, $sessionStorage, validatorFeedbackService, env, LOCALE_MAPPINGS, LOCALE) {
    ...
    if ($stateParams.jobid) {
        ...
        $state.go('dataValidatorLegacyKey', {jobid: $sessionStorage.gbifRunningValidatonJob});
    }
    ...
    function handleValidationSubmitResponse(data) {
        ...
        if ((data.status === 'RUNNING' || data.status === 'ACCEPTED' || data.status === 'NOT_FOUND') && data.jobId) {
            ...
            $timeout(function() {
                if ($state.is('dataValidatorLegacyKey')) {
                ...
                } else if ($state.is('dataValidatorLegacy')) {
                    $state.go('dataValidatorLegacyKey', {jobid: vm.jobId});
                }
            }, 1000);
